...
class TweenableTest: XCTestCase {
    ...
    func testTweenProperty() {
        let xTween = UIView().tween(property: .x(100.0), duration: 1.0)
        XCTAssertFalse(xTween.reversed)
        XCTAssertNil(xTween.callback(get: .complete))
        XCTAssertEqual(Tweener.count, 1)

        let yTween = UIView().tween(property: .y(100.0), duration: 1.0, completion: {})
    ...
    func testTweenProperties() {
        let positionTween = UIView().tween(properties: [.x(100.0), .y(100.0)], duration: 1.0)
        XCTAssertFalse(positionTween.reversed)
        XCTAssertNil(positionTween.callback(get: .complete))
        XCTAssertEqual(Tweener.count, 1)

        let sizeTween = UIView().tween(properties: [.width(100.0), .height(100.0)], duration: 1.0, completion: {})
